...
- Lyrion controls: while a player is on its own Soloist stream, its play,
  pause, stop, and volume controls are forwarded to that Spotify Connect
  session. Controls for other Lyrion sources are not forwarded.
...
1. Create a GitHub Release (e.g. `v0.3.9`) and upload
   `SpotifySoloist-0.3.9.zip` (in this repo) as a release asset. Its
